...
if (process.platform !== "win32") {
  console.log("browser native desktop helper smoke skipped: Windows-only helper");
  process.exit(0);
}

if (!existsSync(helperPath)) {
  throw new Error(`Missing browser native desktop helper: ${helperPath}`);
}

await assertHelper("status", {
  schemaVersion: "browser-native-desktop-helper.v1",
  requestId: "native-helper-smoke-status",
  command: "status",
  timeoutMs: 5_000,
...
const observe = await assertHelper("observe", {
  schemaVersion: "browser-native-desktop-helper.v1",
  requestId: "native-helper-smoke-observe",
  command: "observe",
  timeoutMs: 5_000,
  session: createSession()
});
...
const read = await assertHelper("execute read", {
  schemaVersion: "browser-native-desktop-helper.v1",
  requestId: "native-helper-smoke-execute-read",
  command: "execute",
  timeoutMs: 5_000,
  session: createSession(),
...
const blockedSecret = await runHelper({
  schemaVersion: "browser-native-desktop-helper.v1",
  requestId: "native-helper-smoke-secret",
  command: "execute",
  timeoutMs: 5_000,
  session: createSession(),
...
console.log("browser native desktop helper smoke ok");

function createSession() {
  return {
    id: "native-helper-smoke",
