...
> 이 downstream 프로젝트의 현재 기본값은 `.vibe/config.json` 기준으로 Orchestrator/Planner/Generator/Evaluator 모두 `codex`다. Claude 계열 provider는 사용자가 명시적으로 설정을 바꿀 때만 보조/fallback provider로 취급한다.
...
| `codex` | Windows: `.\.vibe\harness\scripts\run-codex.cmd`; POSIX/Git Bash: `./.vibe/harness/scripts/run-codex.sh -` | **Codex CLI** wrapper 경유. 인증: OAuth (`codex auth login`, 기본) 또는 API 키 (`OPENAI_API_KEY`). 상세: `docs/context/codex-execution.md` |
| `gemini` | Bash 도구 (`gemini "{prompt}"`) | CLI 직접 실행 |

> **⚠️ Provider 호출 규칙**:
> - **Claude 계열** provider → Claude Code의 **Agent 도구** 사용 (model 파라미터 지정)
> - **Codex** → Windows에서는 `.\.vibe\harness\scripts\run-codex.cmd`, POSIX/Git Bash에서는 `./.vibe/harness/scripts/run-codex.sh -` wrapper 경유 CLI 호출. 역할 실행은 `npm run vibe:run-agent -- --provider codex --role <role>`를 우선한다. raw `codex exec` 직접 호출은 Korean Windows 환경에서 mojibake 위험이 있으므로 금지.
