...
- Screen/Vision provider is snapshot-based and has a Windows capture helper, Settings/env configured crop, optional local OCR command hook, bundled OCR runtime packaging, standard Windows Tesseract discovery, bundled tessdata language defaults, OCR-only PNG preprocessing, image hash/change metadata, and app-server image input; visual drag region selection and image-diff thresholds remain future work.
