...
- Screen provider: crop/diff and OCR quality tuning on top of the existing daemon-triggered Windows capture helper, bundled OCR runtime packaging, standard Windows Tesseract discovery, language selection defaults, optional OCR command hook, and direct app-server image input.
