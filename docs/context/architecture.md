...
- Research performance state: schema v4 adds a daemon-owned computer-use eval ledger, perception graph store, structured failure memory, and capability DAG run/node tables. Browser Action, generic capability jobs, OCR, Vision observe jobs, Terminal, and bounded Windows helper capability paths can attach eval evidence through `evalRunId`; the capability runtime auto-creates eval runs when none are supplied and links capability resources into eval resources on final job events.
- Scoped autonomy state: schema v5 adds permission profiles, autonomy runs, capability gaps, generated Toolsmith specs, and generated tool runs. The daemon can detect a missing workflow, evaluate a scoped-YOLO permission profile, materialize bounded runtime tools, require smoke success, execute staged DAG nodes, rerun generated tools from manifests, and link output artifacts back into computer-use eval resources. Current implemented classes include web research to PDF, local document conversion, terminal generated tools, and browser download verification. Broader arbitrary code generation still requires the documented sandboxing and safety hardening.
...
- `npm run smoke:research-performance-architecture`: verify schema v4, unified eval ledger APIs, deterministic ASR command decoding, perception graph target explanation, ROI cascade planning, structured failure memory safety, and capability DAG/eval integration.
