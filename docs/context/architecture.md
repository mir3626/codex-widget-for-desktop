...
- Screen provider: crop/diff on top of the existing daemon-triggered Windows capture helper, bundled OCR runtime packaging, standard Windows Tesseract discovery, language selection defaults, OCR-only PNG preprocessing, optional OCR command hook, and direct app-server image input.
