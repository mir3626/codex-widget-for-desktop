...
- Screen provider: crop/diff and OCR language/model selection on top of the existing daemon-triggered Windows capture helper, bundled OCR runtime packaging, standard Windows Tesseract discovery, optional OCR command hook, and direct app-server image input.
...
- `npm run smoke:ocr-runtime`: verify OCR runtime copy/manifest packaging, standard Windows install discovery, and daemon bundled OCR command resolution.
