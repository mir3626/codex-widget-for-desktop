...
- Scoped autonomy state: schema v5 adds permission profiles, autonomy runs, capability gaps, generated Toolsmith specs, and generated tool runs. The daemon can detect a missing workflow, evaluate a scoped-YOLO permission profile, materialize a reviewed built-in tool template in the runtime workspace, require smoke success, execute the tool, and link output artifacts back into computer-use eval resources. Current generated tools are reviewed templates only, not arbitrary source-code synthesis.
