...
- Vision Context Interface is implemented as the daemon-side preprocessing layer behind Agent screen sharing. It converts capture/session events, provider observations, screenshots, speech text, and semantic facts into a `TaskCapsule`, renders concise capsule markdown, and sends Codex app-server-compatible `UserInput[]` containing text plus selected `localImage` inputs. Raw WebM/audio remain temporary processing material and are deleted after capsule processing.
- A transcription boundary exists with transcript/audio schemas, mock ASR, executable JSON sidecar ASR support through `CODEX_WIDGET_ASR_SIDECAR_COMMAND`, session/user lexicon correction, action-slot confidence scoring, and clarification policy. Local ASR candidate sidecars now cover faster-whisper, whisper.cpp, and Vosk, with CPU-first benchmark wiring and GPU faster-whisper profiles available for dogfood selection.
...
- Vision Context smokes verify capsule generation, app-server `localImage` delivery, raw media deletion, pointer/no-pointer/temporal resolver behavior, mock transcription, lexicon correction, destructive clarification policy, and screen/browser/terminal adapter observation contracts.
- First Vision Context semantic dogfood evidence was collected from the real v1.7.8 `/vibe-review` residual report: daemon `visionContext.*` produced a capsule, deleted raw media, sent one selected screenshot as `localImage`, and correctly held semantic acceptance instead of marking it complete. Full evidence is in `docs/reports/vision-context-dogfood-evidence-2026-05-07.md`.
