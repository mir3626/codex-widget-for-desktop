...
- The Windows native desktop adapter is a bounded browser-window diagnostics and helper-contract boundary, not a placeholder. It can report Windows browser-window availability when `CODEX_WIDGET_BROWSER_ACTION_NATIVE_DESKTOP=1`; if `CODEX_WIDGET_BROWSER_ACTION_NATIVE_DESKTOP_HELPER` points to a bounded helper executable/script, typed native fallback actions are routed through the helper with timeout/error normalization and before/after observation evidence. Live executable browser chrome control remains externally blocked until a scoped UI Automation/native input helper is supplied and validated.
...
- Native desktop executable browser chrome control requires a scoped Windows UI Automation or bounded native input helper. The adapter now has the typed helper invocation contract and mock-covered execution boundary, but it intentionally reports unsupported state when no helper is configured rather than pretending desktop computer-use is complete.
...
- Windows executable UI Automation/browser-chrome control remains blocked on providing and validating a real scoped helper process. The native adapter records the attempted PowerShell browser-window enumeration path, exposes `CODEX_WIDGET_BROWSER_ACTION_NATIVE_DESKTOP_HELPER` for a signed/bounded helper, and smoke-covers the helper JSON contract with a mock executable path.
