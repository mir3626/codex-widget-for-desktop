...
Status: tray/autostart, runtime status, installer builds, native daemon restart supervision, renderer-visible native daemon diagnostics, bundled daemon/Node runtime resources, installed `_up_` resource resolution, resident resource smoke, resident soak smoke, bundled runtime smoke, release exe launch smoke, NSIS install/uninstall plus daemon-restart smoke, and a one-command `release:verify` gate are implemented. Remaining desktop-readiness work is MSI install/uninstall observation, longer soak coverage, and release checklist polish.
