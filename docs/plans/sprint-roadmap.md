...
Status: DOM snapshot ingress, an unpacked browser DOM extension, Screen/Vision snapshot ingress, daemon-triggered Windows screen capture, and explicit terminal command execution are implemented with smoke coverage. Remaining provider work is OCR/direct image input, store packaging, and deeper native integrations.
