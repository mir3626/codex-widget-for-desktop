...
- A Windows PowerShell helper captures the virtual desktop or Settings/env configured crop rectangles as compressed JPEG data, can run a local OCR command, bundled Tesseract runtime, or auto-detected `tesseract` against an OCR-only upscaled PNG, and posts the result to the Screen/Vision endpoint; the widget can trigger that helper through the daemon protocol.
...
- Final browser store account submission, richer full-screen terminal mouse UX, visual drag region selection, and image-diff thresholds remain open.
