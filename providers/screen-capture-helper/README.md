...
- `-OcrScale`: default `2`, scales the OCR-only image before running OCR.
- `-OcrMaxWidth`: default `2400`, caps the OCR-only image width.
...
- `-DisableOcrPreprocess`: send the captured JPEG to OCR instead of an upscaled PNG.
