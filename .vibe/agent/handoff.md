...
`iter-2-sprint-04-provider-packaging`: continue packaging the snapshot providers into user-facing helpers, next with final browser store submission workflow, richer full-screen terminal mouse UX on top of the PTY viewport, crop/diff tuning, an actual multi-hour soak run, and native packaging polish.
...
- Final browser store account submission, richer full-screen terminal mouse UX, crop/diff tuning, and an actual multi-hour/manual soak run remain open for live-service readiness.
